...
listGe=["گ", "كٌـ", "كـ", "كَـ", "كًـ", "كُـ", "كّـ", "كَّـ", "كًّـ", "كُّـ", "كٌّـ"] #If U Want Change This List,Dont Forger to Change listge in line 379
listChe=["چ", "حٍ", "ج", "ح", "حِ", "جِ"]
listPe=["پ", "بِ", "ب", "بٍ", "يِـ", "يٍـ", "ىٍـ", "ىِـ"]
listZhe=["ژ", "رٌ", "رْ", "رّ", "رَْ", "رَّ", "رَ", "رً"]
ge="گ"
che="چ"
pe="پ"
zhe="ژ"
...
def ArFaKey():
        global tr
        LastChar=["‌"," ",".","،",":","؛","!","؟","(",")","'",'"','«','»',"[","]","{","}","<",">","*","÷","×","\n","."]
        def LastCharacter(a,z):
                global tr
                for i in LastChar:
                        First=a+i
                        Last=z+i
                        tr=tr.replace(First,Last)
                if tr[-1] == a:
                        listTr=list(tr)
                        listTr[-1]=z
                        tr=''.join(listTr)
...
class Ui_FarsiNevis(object):
    def setupUi(self, FarsiNevis):
        global listGe,listChe,listPe,listZhe
...
#==========================================================================Lables
    ...
    def retranslateUi(self, FarsiNevis):
        ...
        for i in range(len(listGe)):
                self.cmbGe.setItemText(i, _translate("FarsiNevis", listGe[i]))
        for i in range(len(listChe)):
                self.cmbChe.setItemText(i, _translate("FarsiNevis", listChe[i]))
        for i in range(len(listPe)):
                self.cmbPe.setItemText(i, _translate("FarsiNevis", listPe[i]))
        for i in range(len(listZhe)):
                self.cmbZhe.setItemText(i, _translate("FarsiNevis", listZhe[i]))
...
#=======================================================================cmbKeyBoardAr
    def cmbKeyBoardAr(self):
      listge=["گ", "كٌ", "ك", "كَ", "كً", "كُ", "كّ", "كَّ", "كًّ", "كُّ", "كٌّ"]
      global checkedOrNot,ge,che,pe,zhe
      if checkedOrNot=="1":#0
         ge=listge[self.cmbGe.currentIndex()]
